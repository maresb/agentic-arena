...
1.  **Model Selection: Qwen2.5-VL-72B-Instruct**
    ...
    *   **Quantization**: **Q4_K_M** (~43GB weights) is the agreed baseline. This allows ~53GB of VRAM headroom on the user's 96GB card, which is critical for handling the large KV cache (~20-26GB) generated by high-resolution visual tokens.
...
2.  **Preparation Strategy: Window-Aware Cropping with Delta Optimization**
    ...
    *   **Step 3 (Smart Cropping)**:
        ...
        *   **Edge Case**: If a window is maximized (full 4K) and Step 0 failed, split it into 2 vertical tiles.
...
4.  **Hardware & Implementation Specifics**
    ...
    *   **Platform**: Linux/NixOS.
        *   **Configuration**: `services.ollama.acceleration = "cuda";`
        *   **Capture**: `gnome-screenshot` (primary) or D-Bus portal (Wayland fallback).
        *   **Processing**: `pyvips` for efficient cropping.
...
2.  **Pipeline**:
    ...
    *   **Window Loop**:
        *   `for window in metadata`:
            *   `if window.maximized and step0_failed: split_vertical()`
            *   `else: crop = screenshot.crop(window.rect)`
